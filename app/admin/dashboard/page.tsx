...
export default async function AdminDashboardPage() {
  ...
  return (
    <AppLayout
      ...
      subtitle="Ringkasan penilaian kinerja guru dan status metode MOORA"
      user={session}
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KartuStatistik
            ...
            subtext={totalBelum === 0 ? 'Semua data lengkap' : 'Perlu dinilai'}
            icon={Clock}
          />
          <KartuStatistik
            title="Periode Aktif"
        ...
        {/* Progress Status Penilaian & SPK MOORA */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <h3 className="font-semibold text-sm text-teks-utama">
                Status Kelengkapan Penilaian ({periodeAktif?.namaPeriode || 'Periode'})
              </h3>
              <p className="text-xs text-teks-sekunder mt-0.5">
                {totalSudah} dari {totalGuruAktif} guru aktif telah memiliki nilai lengkap (C1, C2, C3, C4)
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              <Link
                ...
                className="px-3.5 py-1.5 bg-latar-soft hover:bg-hijau-soft border border-hijau-muda rounded-lg text-xs font-semibold text-teks-utama transition-colors"
              >
                Kelola Penilaian
              </Link>
              <Link
                href="/admin/moora"
                className="px-3.5 py-1.5 bg-hijau-utama hover:bg-hijau-hover text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors"
              >
        ...
        {/* Grid 2 Kolom: Tabel Hasil Ranking & Log Aktivitas */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Kolom 1-2: Tabel Ringkasan Ranking */}
          <div className="lg:col-span-2 bg-white rounded-xl border border-hijau-muda overflow-hidden">
            <div className="p-4 border-b border-hijau-muda flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-sm text-teks-utama">
                  Hasil Pemeringkatan MOORA Teratas
                </h3>
              </div>
              <Link
                href="/admin/ranking"
                  ...
                  {hasilRanking.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-6 text-center text-teks-sekunder">
                        Belum ada hasil perhitungan MOORA pada periode ini.
                      </td>
                    </tr>
                  ) : (
                    hasilRanking.slice(0, 5).map((row: any) => (
                      <tr key={row.id} className="hover:bg-hijau-soft/40 transition-colors">
                        ...
                        <td className="py-2.5 px-3 font-semibold">{row.guru.nama}</td>
          ...
          {/* Kolom 3: Log Aktivitas Sistem */}
          <div className="bg-white p-4 rounded-xl border border-hijau-muda space-y-3">
            <h3 className="font-semibold text-sm text-teks-utama">Aktivitas Terakhir</h3>
            <div className="space-y-2.5 text-xs">
              {logAktivitas.length === 0 ? (
                <p className="text-teks-sekunder text-center py-4">Belum ada aktivitas tercatat.</p>
              ) : (
                logAktivitas.map((log: any) => (
                  <div key={log.id} className="p-2.5 rounded-lg bg-latar-soft border border-hijau-muda/50 space-y-1">
                    <div className="flex justify-between items-center text-[10px] text-teks-sekunder">
                      <span className="font-semibold text-teks-utama">{log.user?.nama || 'Sistem'}</span>
                      <span>
                    ...
                    <p className="text-[11px] text-teks-utama">{log.aktivitas}</p>
                  </div>
                ))
              )}
            </div>
