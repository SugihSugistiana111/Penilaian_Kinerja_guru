...
export default function KepsekMonitoringPage() {
  ...
  useEffect(() => {
    const fetchGuru = async () => {
      try {
        ...
        setDaftarGuru(data);
        if (data.length > 0) setSelectedGuruId(data[0].id);
      } catch (e) {
        console.error(e);
      }
    };
  ...
  return (
    <AppLayout
      ...
      subtitle="Analisis grafik perkembangan kompetensi dan nilai preferensi guru antar periode"
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-5 rounded-xl border border-hijau-muda space-y-3">
              <div>
                ...
                <p className="text-[11px] text-teks-sekunder mt-0.5">
                  C1: Kehadiran, C2: Ketepatan Waktu, C3: Perangkat Ajar, C4: Administrasi
                </p>
              </div>
            ...
            {/* Tren Preferensi MOORA */}
            <div className="bg-white p-5 rounded-xl border border-hijau-muda space-y-3">
              <div>
                <h3 className="font-semibold text-sm text-teks-utama flex items-center gap-2">
                  <LineChartIcon className="w-4 h-4 text-hijau-utama" />
                  <span>Nilai Preferensi MOORA (Yi)</span>
                </h3>
                <p className="text-[11px] text-teks-sekunder mt-0.5">
                  Nilai optimasi preferensi antar periode
                </p>
              </div>
