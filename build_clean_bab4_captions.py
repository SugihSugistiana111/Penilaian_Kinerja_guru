...
def build_all_50_captions():
    ...
    # 1. Remove all paragraphs starting with "Gambar 4." in BAB IV (after element 780)
    ...
    # 2. Find all drawings in BAB IV (after element 780)
    drawings = []
    for elem in list(body):
        xml_s = etree.tostring(elem).decode('utf-8', errors='ignore')
    ...
    print(f"Total drawings in BAB IV: {len(drawings)} (Expected: {len(target_captions)})")
