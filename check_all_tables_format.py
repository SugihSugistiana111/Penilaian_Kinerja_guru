import docx
from lxml import etree
from docx.oxml.ns import qn

doc = docx.Document('SKRIPSI.docx')
body_elements = list(doc._body._body)

def get_text(elem):
    return ''.join(t.text or '' for t in elem.iter('{http://schemas.openxmlformats.org/wordprocessingml/2006/main}t'))

print("=== CHECKING ALL BAB IV TABLES (Tabel 4.1+) ===")
count = 0
for i, elem in enumerate(body_elements):
    if i < 784:
        continue
    tag = elem.tag.split('}')[-1]
    if tag != 'p':
        continue
    xml = etree.tostring(elem).decode('utf-8', errors='ignore')
    if 'SEQ Tabel' not in xml:
        continue
    
    count += 1
    text = get_text(elem)
    pPr = elem.find(qn('w:pPr'))
    style = ''
    outline = ''
    num = ''
    align = ''
    before = ''
    after = ''
    if pPr is not None:
        pStyle = pPr.find(qn('w:pStyle'))
        if pStyle is not None:
            style = pStyle.get(qn('w:val'), '')
        outline_el = pPr.find(qn('w:outlineLvl'))
        if outline_el is not None:
            outline = outline_el.get(qn('w:val'), '')
        num_el = pPr.find(qn('w:numPr'))
        if num_el is not None:
            num = 'has_numPr'
        jc_el = pPr.find(qn('w:jc'))
        if jc_el is not None:
            align = jc_el.get(qn('w:val'), '')
        sp_el = pPr.find(qn('w:spacing'))
        if sp_el is not None:
            before = sp_el.get(qn('w:before'), '')
            after = sp_el.get(qn('w:after'), '')
            
    runs = elem.findall(qn('w:r'))
    fonts = set()
    sizes = set()
    for r in runs:
        rPr = r.find(qn('w:rPr'))
        if rPr is not None:
            rf = rPr.find(qn('w:rFonts'))
            if rf is not None:
                fonts.add(rf.get(qn('w:ascii'), ''))
            sz = rPr.find(qn('w:sz'))
            if sz is not None:
                sizes.add(sz.get(qn('w:val'), ''))
                
    is_heading = ('Heading' in style) or (outline != '') or (num != '')
    status = "ERROR_HEADING" if is_heading else "OK"
    print(f"T[{count:02d}] {status:13} style='{style}' jc='{align}' sp=({before},{after}) fonts={list(fonts)} sz={list(sizes)} | {text[:45]}")
